...
import {   edit_cv, get_one_cv } from '../../../../redux/Action/CvAction';
import { ChevronDownIcon, HamburgerIcon } from '@chakra-ui/icons'
import {   FormControl, FormLabel, Input,CheckboxGroup,
  Stack,
...
const EditCv = () => {
  ...
  const {id}=useParams()
   const cv = useSelector((state) =>state.cvreducer.currentCv);

   useEffect(() => {
    dispatch(get_one_cv(id))
   }, [dispatch])
  ...
  const [Firstname, setFirstname] = useState(cv.Firstname);
        ...
        const [numberphone, setNumberphone] = useState(cv?.numberphone);
        const [adress, setAdress] = useState(cv?.adress);
        const [aboutus, setAboutus] = useState(cv?.aboutus);
        const [datedebut, setDatadebut] = useState(cv?.datedebut);
        const [datafin, setDatafin] = useState(cv?.datafin);
        const [formation, setFormation] = useState(cv?.formation);
        const [competance, setCompetance] = useState(cv?.competance);
        const [niveau, setNiveau] = useState(cv?.niveau);
        const [etablisment, setEtablisment] = useState(cv?.etablisment);
        const [ville, setVille] = useState(cv?.ville);
        const [profile, setProflie] = useState(cv?.profile);
        const [Post, setPost] = useState(cv?.Post);
        const [Employeur, setEmployeur] = useState(cv?.Employeur);
        const [langue, setLangue] = useState(cv?.langue);
        const [certificat, setCertificat] = useState(cv?.certificat);
        const [descripcertif,setDescripcertif]=useState(cv?.descripcertif)
        const [datefincertif,setDatefincertif]=useState(cv?.datefincertif)
        const [datedebcertif,setDatedebcertif]=useState(cv?.datedebcertif)
        const [rating, setRating] = useState(cv?.rating);
  const [descriptioncertficat, setDescriptioncertficat] = useState(cv?.descriptioncertficat);
...
const navigate=useNavigate()
    ...
    const handleSubmit = (e) => {
          e.preventDefault();
          dispatch(edit_cv(cv.id,{Firstname,Lastname,email,numberphone,adress,aboutus,
            datedebut,datafin,formation,competance,niveau,etablisment,ville,Post
          ,langue,certificat,datefincertif,datedebcertif,descripcertif}));
